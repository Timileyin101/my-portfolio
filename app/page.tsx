...
/**
 * Represents a project entity from Firebase
 */
interface Project {
  id: string;
  title: string;
  ...
  media?: MediaItem[]; // New: array of media objects
  mediaUrl?: string;   // Legacy: single media URL
  liveLink?: string;   // For frontend projects
...
/**
 * Submit status for contact form
 */
...
/**
 * Tab configuration for All Projects Modal
 */
interface TabConfig {
  id: 'graphics' | 'motion' | 'frontend';
  label: string;
...
/**
 * Props for ProjectPreviewModal component
 */
...
/**
 * Props for AllProjectsModal component
 */
...
export default function PortfolioPage(): JSX.Element {
  ...
  const [showAllProjectsModal, setShowAllProjectsModal] = useState<boolean>(false);
   ...
   */
  const fetchProjectsFromFirebase = async (): Promise<void> => {
    try {
  ...
  // Static data
  const skills: string[] = ['Figma', 'After Effects', 'Photoshop', 'Illustrator', 'React', 'Next.js', 'Tailwind'];
   ...
   */
  async function submitContact(e: React.FormEvent<HTMLFormElement>): Promise<void> {
    e.preventDefault();
  ...
  const whatsappNumber: string | undefined = process.env.NEXT_PUBLIC_WHATSAPP_NUMBER;
  ...
  const ProjectPreviewModal: React.FC<ProjectPreviewModalProps> = ({ project, onClose }) => {
    ...
    /**
     * Get media items from project (supports both new array format and legacy single URL)
     */
    ...
    const currentMedia: MediaItem | undefined = mediaItems[currentMediaIndex];
    const hasMultipleMedia: boolean = mediaItems.length > 1;

    const goToNext = (): void => setCurrentMediaIndex((prev) => (prev + 1) % mediaItems.length);
    const goToPrevious = (): void => setCurrentMediaIndex((prev) => (prev - 1 + mediaItems.length) % mediaItems.length);
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 p-4" onClick={onClose}>
        <div
          className="relative w-full max-w-2xl max-h-[95vh] overflow-auto bg-slate-900 rounded-2xl border border-slate-800"
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1">
                ...
                <div className="flex items-center gap-3 mt-2">
                  <span className="inline-block px-2.5 py-1 bg-blue-500/20 text-blue-400 text-xs font-semibold rounded-md uppercase">
                    {project.type}
                  </span>
                  {hasMultipleMedia && (
  ...
  const AllProjectsModal: React.FC<AllProjectsModalProps> = ({ onClose, onSelectProject }) => {
    const [activeTab, setActiveTab] = useState<'graphics' | 'motion' | 'frontend'>('graphics');
    ...
    const filteredProjects: Project[] = projects.filter(project => project.type === activeTab);

    /**
     * Gets the first media item from a project (supports both formats)
     */
    const getFirstMedia = (project: Project): MediaItem | null => {
      if (project.media && Array.isArray(project.media) && project.media.length > 0) {
              ...
              {tabs.map((tab) => {
                ...
                return (
                  <button 
                    ...
                    className={`relative flex items-center gap-2 px-6 py-4 font-medium text-sm transition-all ${
                  ...
                  >
                    ...
                    <span className={`ml-1 px-2 py-0.5 rounded-full text-xs ${
                      isActive ? 'bg-blue-500/20 text-blue-400' : 'bg-slate-800 text-gray-500'
                    }`}>
                      {count}
                    </span>
            ...
            <div className="p-6">
              {loadingProjects ? (
                ...
                <div className="text-center py-12">
                  ...
                  <h3 className="text-lg font-semibold text-gray-300 mb-2">No {activeTab} projects yet</h3>
                  <p className="text-gray-500">Check back soon for new {activeTab} work</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {filteredProjects.map((project) => {
                    ...
                    return (
                      <article 
                        key={project.id} 
                        className="bg-slate-800 rounded-xl overflow-hidden hover:ring-2 hover:ring-blue-500 transition-all cursor-pointer group shadow-lg hover:shadow-blue-500/20" 
                      ...
                      >
                        <div className="relative aspect-video bg-black flex items-center justify-center overflow-hidden">
                          ...
                          <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center gap-3">
                            <span className="flex items-center gap-2 text-white font-medium px-4 py-2 bg-blue-600 rounded-lg">
                              <Eye size={18} />
                              View Project
                            </span>
                          </div>
                        ...
                        <div className="p-6">
                          <div className="flex items-start justify-between gap-4">
                            <div className="flex-1">
                              ...
                              <p className="text-blue-400 text-sm uppercase tracking-wide mt-1 flex items-center gap-2">
                                {project.type}
                                {project.type === 'frontend' && project.liveLink && <Globe size={14} />}
                              </p>
                            </div>
                            <ArrowRight className="text-blue-500 group-hover:translate-x-1 transition-transform flex-shrink-0 mt-1" size={20} />
                          </div>
                          <p className="mt-3 text-gray-300 text-sm leading-relaxed line-clamp-2">
                            {project.description}
                          </p>
                        </div>
                      </article>
          ...
          {loadingProjects ? (
            ...
            >
              {projects.slice(0, 6).map((project) => {
                const firstMedia: MediaItem | null =
          ...
          <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <form onSubmit={submitContact} className="space-y-4">
                  <input
                    ...
                    className={`w-full p-3 rounded-md border ${
                      dark ? 'border-slate-700 bg-black/30' : 'border-slate-200 bg-slate-50'
            ...
            <motion.div
              className={`p-8 rounded-2xl border ${
                dark ? "border-slate-800 bg-gray-900/50" : "border-slate-200 bg-white"
              }`}
              variants={fadeUp}
            >
